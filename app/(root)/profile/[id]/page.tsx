      ...
      <Tabs defaultValue="posts">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="posts" className="rounded-lg bg-white text-lg font-semibold shadow-md">
            ...
            Posts<span className="ml-8 text-slate-600">{vendors.length}</span>
          </TabsTrigger>
          <TabsTrigger value="photos" className="rounded-lg bg-white text-lg font-semibold shadow-md">
            <ImageIcon className="mr-2" />
            Photos<span className="ml-8 text-slate-600">{media.length}</span>
          </TabsTrigger>
        </TabsList>
                    ...
                    <div key={vendor.vendor_id}>
                      ...
                      <div className="flex  justify-between">
                        ...
                        <div className="flex  justify-between gap-x-1">
                          ...
                          <span>
                            {' '}
                            {vendor.avgRating} <span className="font-normal text-slate-400">({vendor.totalRatings})</span>
                          </span>
                        </div>
                      </div>
                  ...
                  media.map((url, index) => {
                    return (
                      <div key={index} className="relative">
                        <img
                          src={`https://wjyimkeequncdarvitza.supabase.co/storage/v1/object/public/uploads/${user?.id}/${url.name}`}
                          alt={`Image ${index}`}
